...
CREATE TABLE chunks (
  ...
  chunk_size_mb INT UNSIGNED NOT NULL,
...
FOR EACH ROW UPDATE totals SET total = total + NEW.chunk_size_mb WHERE id=1;

CREATE TRIGGER chunks_ad AFTER DELETE ON chunks
FOR EACH ROW UPDATE totals SET total = total - OLD.chunk_size_mb WHERE id=1;

CREATE TRIGGER chunks_au AFTER UPDATE ON chunks
FOR EACH ROW UPDATE totals SET total = total + (NEW.chunk_size_mb - OLD.chunk_size_mb) WHERE id=1;
...
CREATE PROCEDURE purge_chunks_to_limit(IN p_limit_mb INT UNSIGNED)
proc:BEGIN
  ...
  SET v_excess = IF(v_total > p_limit_mb, v_total - p_limit_mb, 0);
  ...
  WITH s AS (
    SELECT
      chunk_id,
      SUM(chunk_size_mb) OVER (ORDER BY chunk_created_at, chunk_id) AS run_sum,
      COALESCE(
        SUM(chunk_size_mb) OVER (
